...
test.describe("Rechirps", () => {
	...
	test("should rechirp a post", async ({ page }) => {
		...
		await expect(page.locator('button[title="Undo rechirp"]').first()).toBeVisible();
	});
	...
	test("should update rechirp count after rechirping", async ({ page }) => {
		...
		const newCount = Number.parseInt((await updatedButton.locator("span").textContent()) ?? "0");
		expect(newCount).toBe(initialCount + 1);
	});

	test("should show rechirp count decrement after undoing", async ({ page }) => {
		await page.goto("/users/bob", { waitUntil: "networkidle" });
		await waitForHydration(page);
		...
		const afterRechirpCount = Number.parseInt(
			(await page.locator('button[title="Undo rechirp"]').first().locator("span").textContent()) ??
				"0",
		);
		...
		const afterUndoCount = Number.parseInt(
			(await page.locator('button[title="Rechirp"]').first().locator("span").textContent()) ?? "0",
		);
	...
	test("should show rechirped post on reposter profile", async ({ page }) => {
		...
		await expect(page.locator("article").filter({ hasText: "rechirped" }).first()).toBeVisible();
	});

	test("should show original author on rechirped post in profile", async ({ page }) => {
		// Rechirp Bob's first post as Alice
		await page.goto("/users/bob", { waitUntil: "networkidle" });
		await waitForHydration(page);

		// Grab the content of the first rechirpable post for verification
		const firstPost = page.locator("article").filter({ has: page.locator('button[title="Rechirp"]') }).first();
		const postContent = (await firstPost.locator("a[href*='/posts/']").first().textContent()) ?? "";
		...
		const rechirpedCard = page.locator("article").filter({ hasText: "rechirped" }).first();
