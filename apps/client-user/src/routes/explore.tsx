...
function ExplorePage() {
  ...
  const fetchExploreFeed = useCallback(
    (options: { limit: number; offset: number }) =>
      getExploreFeed({ data: options }),
    [],
  );

  const { posts, loading, loadingMore, hasMore, sentinelRef } =
    useInfiniteScroll('explore', fetchExploreFeed);
      ...
      <div {...stylex.props(styles.pageHeader)}>
        ...
        <div>
          ...
          <p {...stylex.props(styles.pageSubtitle)}>
            Discover new posts from everyone
          </p>
        </div>
      </div>
