...
function HomePage() {
  ...
  // Stable fetch function – defined outside of render so the hook's effect
  // dependency array stays satisfied without adding getHomeFeed as a dep.
  const fetchHomeFeed = useCallback(
    (options: { limit: number; offset: number }) =>
      getHomeFeed({ data: options }),
    [],
  );

  const { posts, loading, loadingMore, hasMore, sentinelRef, reload } =
    useInfiniteScroll('home', fetchHomeFeed);
